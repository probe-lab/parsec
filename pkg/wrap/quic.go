...
func NewQuicTransport() (*QuicTransport, func(key ic.PrivKey, connManager *quicreuse.ConnManager, psk pnet.PSK, gater connmgr.ConnectionGater, rcmgr network.ResourceManager) (*QuicTransport, error)) {
	q := &QuicTransport{
		// Notifier: newNotifier(models.DialTransportQuic),
		Notifier: newNotifier("quic"),
	}

	return q, func(key ic.PrivKey, connManager *quicreuse.ConnManager, psk pnet.PSK, gater connmgr.ConnectionGater, rcmgr network.ResourceManager) (*QuicTransport, error) {
		trpt, err := quic.NewTransport(key, connManager, psk, gater, rcmgr)
		q.trpt = trpt
		return q, err
	}
}
